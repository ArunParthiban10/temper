...
pub extern "C" fn run(_ctx_ptr: i32, _ctx_len: i32) -> i32 {
    let result = (|| -> Result<(), String> {
        ...
        let channel_id = fields
            ...
            .and_then(|v| v.as_str())
            .unwrap_or("");
        let default_agent_config = fields
            .get("default_agent_config")
            .and_then(|v| v.as_str())
            .unwrap_or("{}");
        ...
        let agent_id = if let Some(session) = existing_session {
            ...
            let agent_id = session
                ...
                .unwrap_or_default()
                .to_string();
            resume_session(&ctx, &temper_api_url, &ctx.tenant, &session_id)?;
            steer_existing_agent(&ctx, &temper_api_url, &ctx.tenant, &agent_id, content)?;
            agent_id
        } else {
            let route = find_route(&ctx, &temper_api_url, &ctx.tenant, channel_id)?;
            let route_config = route
                .as_ref()
                .and_then(|value| value.get("AgentConfig"))
                .and_then(Value::as_str)
                .filter(|value| !value.trim().is_empty())
                .unwrap_or(default_agent_config);
            let route_soul_id = route
                .as_ref()
                .and_then(|value| value.get("SoulId"))
                .and_then(Value::as_str)
                .unwrap_or("");
...
fn find_active_session(
    ...
    let sessions = list_entities(ctx, &format!("{temper_api_url}/tdata/ChannelSessions"), tenant)?;
    Ok(sessions.into_iter().find(|session| {
        session.get("Status").and_then(Value::as_str) == Some("Active")
            && session.get("ChannelId").and_then(Value::as_str).unwrap_or("") == channel_id
            && session.get("ThreadId").and_then(Value::as_str).unwrap_or("") == thread_id
            && session.get("AuthorId").and_then(Value::as_str).unwrap_or("") == author_id
    }))
}
...
fn resume_session(
    ...
    let url = format!(
        "{temper_api_url}/tdata/ChannelSessions('{session_id}')/Temper.OpenClaw.ChannelSession.Resume"
    );
...
fn find_route(
    ...
    Ok(routes.into_iter().find(|route| {
        route.get("Status").and_then(Value::as_str) == Some("Active")
            && {
                let route_channel_id = route.get("ChannelId").and_then(Value::as_str).unwrap_or("");
...
fn create_agent_from_route(
    ...
    let configure_body = json!({
        ...
        "model": config.get("model").and_then(Value::as_str).unwrap_or("mock"),
        "provider": config.get("provider").and_then(Value::as_str).unwrap_or("mock"),
        ...
        "sandbox_url": config.get("sandbox_url").and_then(Value::as_str).unwrap_or("http://127.0.0.1:9999"),
...
fn create_session(
    ...
    let create_url = format!(
        "{temper_api_url}/tdata/ChannelSessions('{session_id}')/Temper.OpenClaw.ChannelSession.Create"
    );
    let body = json!({
