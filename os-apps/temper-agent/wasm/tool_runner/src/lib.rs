...
fn evaluate_before_hooks(
    ...
    for hook in hooks {
        let action = hook
            ...
            .and_then(Value::as_str)
            .unwrap_or("log");
        let name = hook.get("Name").and_then(Value::as_str).unwrap_or("hook");
        match action {
            "block" => {
...
fn load_matching_hooks(
    ...
    let hooks = parsed
        ...
        .into_iter()
        .filter(|hook| {
            hook.get("Status").and_then(Value::as_str) == Some("Active")
                && hook.get("SoulId").and_then(Value::as_str).unwrap_or("") == soul_id
                && hook.get("HookType").and_then(Value::as_str).unwrap_or("") == hook_type
                && hook_matches(
                    hook.get("ToolPattern")
                        .and_then(Value::as_str)
                        .unwrap_or(".*"),
                    tool_name,
                )
...
fn execute_entity_tool(
    ...
    match tool_name {
        ...
        "recall_memory" => {
            ...
            if resp.status == 200 {
                ...
                let memories = parsed
                    ...
                    .into_iter()
                    .filter(|mem| {
                        mem.get("Status").and_then(Value::as_str) == Some("Active")
                            && mem.get("SoulId").and_then(Value::as_str).unwrap_or("") == soul_id
                            && (mem
                                .get("Key")
                                .and_then(Value::as_str)
                                .unwrap_or("")
                ...
                if memories.is_empty() {
                    ...
                    for mem in &memories {
                        let k = mem.get("Key").and_then(|v| v.as_str()).unwrap_or("?");
                        let c = mem.get("Content").and_then(|v| v.as_str()).unwrap_or("");
                        let t = mem.get("MemoryType").and_then(|v| v.as_str()).unwrap_or("?");
        ...
        "list_agents" => {
            ...
            if resp.status == 200 {
                ...
                let agents = parsed
                    ...
                    .into_iter()
                    .filter(|agent| {
                        agent.get("ParentAgentId").and_then(Value::as_str).unwrap_or("") == parent_id
                    })
                    .collect::<Vec<_>>();
                if agents.is_empty() {
                ...
                Err(format!("list_agents failed (HTTP {})", resp.status))
            }
        }
        "abort_agent" => {
            let agent_id = input.get("agent_id").and_then(|v| v.as_str()).ok_or("abort_agent: missing 'agent_id'")?;
            let url = format!(
                "{temper_api_url}/tdata/TemperAgents('{agent_id}')/Temper.Agent.TemperAgent.Cancel"
            );
            let resp = ctx.http_call("POST", &url, &odata_headers(tenant), "{}")?;
            if resp.status >= 200 && resp.status < 300 {
                Ok(format!("Agent {agent_id} cancelled."))
            } else {
        ...
        "steer_agent" => {
            ...
            let current_url = format!("{temper_api_url}/tdata/TemperAgents('{agent_id}')");
            let current = ctx.http_call("GET", &current_url, &odata_headers(tenant), "")?;
            let existing = if current.status == 200 {
                let parsed: Value = serde_json::from_str(&current.body).unwrap_or_else(|_| json!({}));
                parsed
            ...
            if resp.status >= 200 && resp.status < 300 {
                Ok(format!("Steering message sent to agent {agent_id}."))
            } else {
