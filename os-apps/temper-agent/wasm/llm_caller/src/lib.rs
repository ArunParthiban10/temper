...
fn load_soul_content(
    ...
    let content_file_id = parsed.get("ContentFileId").and_then(|v| v.as_str()).unwrap_or("");
...
fn load_skills_block(
    ...
    for skill in &skills {
        ...
        let file_id = skill.get("ContentFileId").and_then(|v| v.as_str()).unwrap_or("");
...
fn load_memory_block(
    ...
    for mem in &memories {
        let key = mem.get("Key").and_then(|v| v.as_str()).unwrap_or("unknown");
        let content = mem.get("Content").and_then(|v| v.as_str()).unwrap_or("");
        let mem_type = mem.get("MemoryType").and_then(|v| v.as_str()).unwrap_or("reference");
